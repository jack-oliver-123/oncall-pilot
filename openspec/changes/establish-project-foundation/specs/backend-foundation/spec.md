...
后端 SHALL 在 `/health` 提供进程存活检查，成功响应 MUST 为 HTTP 200 和 `{"status":"ok"}`，该检查 MUST 不探测数据库或外部服务。

#### Scenario: 调用健康检查
- **WHEN** 客户端向已创建的应用请求 `/health`
- **THEN** 返回 HTTP 200 和 `{"status":"ok"}`
...
foundation SHALL 提供空的异步迁移环境，但 MUST 不创建业务模型、迁移 revision 或数据库文件；只有显式迁移命令 MAY 加载数据库配置和建立连接。
