"""持久任务 metadata；时间使用固定 UTC RFC3339 文本以支持一致排序。"""
...
background_job_events = sa.Table(
...
sa.Index(
    ...
    background_jobs.c.owner_user_id,
    background_jobs.c.status,
    background_jobs.c.available_at,
)
sa.Index("ix_background_jobs_lease", background_jobs.c.status, background_jobs.c.lease_expires_at)
